...
func (a *api) handleListLanes(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{jsonKeyError: "failed to list lanes"})
		return
	}
...
func (a *api) createLaneFromRequest(c *gin.Context, req createLaneRequest) { //nolint:gocyclo,funlen // multi-branch orchestration; keep under threshold when rewriting
	principal, ok := a.requirePrincipal(c)
	if !ok {
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{jsonKeyError: "failed to check secrets"})
		return
	}

	if !agents.AgentAvailable(provider, setSecrets) {
		c.JSON(http.StatusBadRequest, gin.H{
			jsonKeyError: "required secret is not set for this agent; configure Settings → Secrets",
		})
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{jsonKeyError: clientErrorForModelSelection(err)})
		return
	}
	...
	if projectID != nil { //nolint:nestif // nested validation/orchestration; flatten when rewriting
		env, envErr := a.store.GetProjectEnvironment(c.Request.Context(), *projectID)
		if envErr != nil {
			if errors.Is(envErr, store.ErrNotFound) {
				c.JSON(http.StatusBadRequest, gin.H{
					jsonKeyError: "project has no Project Environment; generate or save one first",
				})
		...
		if env.ValidationStatus != models.EnvironmentValidationValid {
			c.JSON(http.StatusBadRequest, gin.H{
				jsonKeyError: "project environment is not valid; validate it first",
			})
		...
		if env.ValidationStatus != models.EnvironmentValidationValid {
			c.JSON(http.StatusBadRequest, gin.H{
				jsonKeyError: "scratch environment is not valid; validate it first",
			})
	...
	if imageRef == nil || *imageRef == "" {
		if a.runtime == nil {
			c.JSON(http.StatusBadRequest, gin.H{jsonKeyError: "environment has no validated image"})
			return
		}
		...
		if buildErr != nil {
			c.JSON(http.StatusUnprocessableEntity, gin.H{jsonKeyError: "failed to build lane image", "detail": buildErr.Error()})
			return
		}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{jsonKeyError: "failed to create lane"})
		return
	}

	if a.runtime != nil { //nolint:nestif // nested validation/orchestration; flatten when rewriting
		...
		if secErr != nil {
			...
			c.JSON(http.StatusInternalServerError, gin.H{jsonKeyError: "failed to load secrets for lane"})
...
func (a *api) cloneProjectIntoLane(ctx context.Context, runtimeID string, project models.Project) error {
	...
	if repo == "" {
		return errors.New("project has no github_full_name")
	}
...
func (a *api) handleDestroyLane(c *gin.Context) { //nolint:gocyclo // multi-branch orchestration; keep under threshold when rewriting
	principal, ok := a.requirePrincipal(c)
	if !ok {
		return
	}
...
func (a *api) handlePatchLane(c *gin.Context) { //nolint:gocyclo,funlen // multi-branch orchestration; keep under threshold when rewriting
	principal, ok := a.requirePrincipal(c)
	if !ok {
		return
	}
	...
	if a.hub != nil && a.hub.IsTurnBusy(lane.ID) {
		c.JSON(http.StatusConflict, gin.H{jsonKeyError: "cannot change agent settings while a turn is running"})
		return
	}
	...
	if req.AgentProvider != nil {
		...
		if !agents.AgentAvailable(provider, setSecrets) {
			c.JSON(http.StatusBadRequest, gin.H{jsonKeyError: "required secret is not set for this agent"})
			return
		}
	...
	if selErr != nil {
		c.JSON(http.StatusBadRequest, gin.H{jsonKeyError: clientErrorForModelSelection(selErr)})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{jsonKeyError: "failed to update lane"})
		return
	}

	if providerChanged { //nolint:nestif // nested validation/orchestration; flatten when rewriting
		payload, err := json.Marshal(map[string]any{
			...
			jsonKeyUserID:      principal.User.ID,
			"at":               time.Now().UTC(),
		})
		if err != nil {
...
func (a *api) resolveCreateModelSelection(
	...
	if source == "" {
		...
		if !ok {
			return agents.ModelSelection{}, errors.New("no model source available for agent")
		}
	}

	if !agents.SourceAllowedForAgent(provider, source, setSecrets) {
		return agents.ModelSelection{}, errors.New(errModelSource)
	}
...
func (a *api) laneJSON(l models.Lane) gin.H {
	...
	out := gin.H{
		...
		jsonKeyOrganizationID:       l.OrganizationID,
		...
		jsonKeyStatus:               l.Status,
		"turn_running":              turnRunning,
		jsonKeyCreatedAt:            l.CreatedAt,
		jsonKeyUpdatedAt:            l.UpdatedAt,
	}
